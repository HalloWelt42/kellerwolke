...
#
# Der Dienst ruft schlicht ./start.sh start / stop - die bewaehrte Startlogik
...
if ! command -v systemctl >/dev/null 2>&1; then
  ...
  echo "dort wird die Anwendung mit ./start.sh gestartet."
  exit 0
fi
...
Type=oneshot
RemainAfterExit=yes
# start.sh legt die Prozesse per nohup im Hintergrund ab und kehrt zurueck;
...
ExecStart=${DIR}/start.sh start
ExecStop=${DIR}/start.sh stop
TimeoutStartSec=600
